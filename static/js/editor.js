...
${html}
    ...
    if (clearBtn) {
        clearBtn.addEventListener('click', () => {
            if (confirm('Are you sure you want to clear all generated content?')) {
                ...
                if (showRefBtn) showRefBtn.style.display = 'none';
                tagDetailsPanel.style.display = 'none';
    ...
    function loadReferenceImage() {
        ...
        if (savedImage && referenceImg) {
            referenceImg.src = savedImage;
    ...
    if (toggleRefBtn) {
        toggleRefBtn.addEventListener('click', () => {
            ...
            if (showRefBtn) showRefBtn.style.display = 'flex';
        });
    }
    ...
    if (showRefBtn) {
        ...
        showRefBtn.style.alignItems = 'center';
        showRefBtn.style.justifyContent = 'center';
    }

    // Draggable Reference Image Logic
    const refHeader = document.getElementById('refHeader');
    if (refHeader && referenceContainer) {
        ...
        refHeader.addEventListener('mousedown', (e) => {
            ...
            if (e.target === refHeader || refHeader.contains(e.target)) {
                isDragging = true;
                refHeader.style.cursor = 'grabbing';
            }
        });
        ...
        document.addEventListener('mousemove', (e) => {
            if (isDragging) {
                ...
                referenceContainer.style.transform = `translate3d(${xOffset}px, ${yOffset}px, 0)`;
            }
        });

        document.addEventListener('mouseup', () => {
            isDragging = false;
            if (refHeader) refHeader.style.cursor = 'grab';
        });
    }

    // Wiring Preview Controls
    ...
    if (refreshBtn) {
        refreshBtn.addEventListener('click', () => {
            ...
            setTimeout(() => refreshBtn.style.transform = 'scale(1.05)', 100);
        });
    }
    ...
    if (toggleDeviceBtn) {
        ...
        toggleDeviceBtn.addEventListener('click', () => {
            ...
            isMobile = !isMobile;
            if (isMobile) {
                previewBox.style.maxWidth = '375px';
                previewBox.style.margin = '0 auto';
                toggleDeviceBtn.style.color = '#2563eb';
                toggleDeviceBtn.style.background = '#eff6ff';
            } else {
                previewBox.style.maxWidth = '100%';
                previewBox.style.margin = '0';
                toggleDeviceBtn.style.color = 'var(--text-primary)';
                toggleDeviceBtn.style.background = 'white';
            }
        });
    }
